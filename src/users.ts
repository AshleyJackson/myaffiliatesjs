...
export async function get_users_by_ids(user_ids: string[]) {
  const request = await axios.get(`/feeds.php?FEED_ID=2&USER_IDS=${user_ids.join(',')}`)
  return request.data
}

type user_details_allowed<ArrType> = {
  user_ids: number[];
  username?: string;
  email?: string;
}

export async function get_users_details(user_details: user_details_allowed<string[]>) {
  const request =  await axios.get('', {
    params: {
      FEED_ID: 2,
      USER_IDS: parseInt(user_details.user_ids.join(',')),
      DETAIL_NAME: 'first_name'
    }
  })
